...
class BST<Key extends Comparable<Key>, Value> {
    ...
    private static void perfect(BST bst, String[] keys) {
        Arrays.sort(keys);
        perfect(bst, keys, 0, keys.length - 1);
        StdOut.println();
    }

    // precondition: a[lo..hi] is sorted
    private static void perfect(BST bst, String[] keys, int low, int high) {
        if (high < low) return;
        int mid = low + (high - low) / 2;
        bst.put(keys[mid], mid);
        StdOut.print(keys[mid] + " ");
        perfect(bst, keys, low, mid - 1);
        perfect(bst, keys, mid + 1, high);
    }
    ...
    public static void main(String[] args) {
        ...
        BST<String, Boolean> bst = new BST<String, Boolean>();
        perfect(bst, keys);
    }
}
